...
const formSchema = z.object({
  ...
  fuelLevelOut: z.coerce.number().min(0).max(100, 'Nível de combustível inválido.'),
  dashboardPhotoUrl: z.string().url('É obrigatório tirar a foto do painel (combustível 1).'),
  dashboardPhotoUrl2: z.string().url('É obrigatório tirar a foto do painel (combustível 2).'),
...
export function ChecklistFormHeavy() {
  ...
  const photoFields: { key: PhotoKey; label: string }[] = [
    ...
    { key: 'dashboardPhotoUrl2', label: 'Painel (Combustível 2)' },
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>
                Se algum item foi marcado como "Avaria", detalhe aqui.
              </FormDescription>
              <FormMessage />
            </FormItem>
          )}
